...
export default function PartnerReferralLinkCard() {
    ...
    const [{ data: referralCode }, { data: details }, { data: requests }] =
      await Promise.all([
        ...
        supabase
          ...
          .select(
            "id,requested_amount,approved_amount,currency,status,admin_note,requested_at,paid_at"
          )
          .eq("partner_id", summary.partner_id)
  ...
  async function savePayoutAccount() {
    ...
    const { error } = await supabase.rpc(
      "update_partner_payout_details",
      {
        ...
        p_account_number: cleanAccountNumber,
      }
    );
  ...
  async function requestPayout() {
    ...
    if (amount < minimum) {
      alert(
        `The minimum referral payout is NGN ${minimum.toLocaleString()}.`
      );
      return;
    }
    ...
    if (amount > available) {
      alert(
        "The requested amount is higher than your available commission."
      );
      return;
    }
    ...
    const confirmed = window.confirm(
      `Request NGN ${amount.toLocaleString()} referral payout?`
    );
    ...
    if (authData.user) {
      await loadPartnerData(authData.user.id);
    }
  ...
  return (
    <section className="mt-7 rounded-2xl border border-slate-800 bg-slate-900 p-5 text-white">
      ...
      <p className="mt-2 text-sm leading-6 text-slate-400">
        Anyone who registers or signs in through this link is attached
        to you automatically for eligible purchases.
      </p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
        ...
        <p>
          ...
          <strong>{partner.conversion_count ?? 0}</strong>
        </p>
        ...
        <p>
          ...
          <strong>
            NGN{" "}
            {Number(
              partner.available_commission || 0
            ).toLocaleString()}
          </strong>
        </p>
        ...
        <p>
          ...
          <strong>
            NGN{" "}
            {Number(partner.paid_commission || 0).toLocaleString()}
          </strong>
        </p>
      </div>
      ...
      <div className="mt-6 border-t border-slate-800 pt-5">
        ...
        <p className="mt-1 text-sm text-slate-400">
          Save the bank account where Fidelity Traders Hub should send
          your approved commission.
        </p>
        ...
        <button
          ...
          disabled={savingBank}
          className="mt-3 rounded-xl border border-slate-700 px-4 py-3 text-sm font-semibold disabled:opacity-50"
        >
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row">
          <input
            ...
            type="number"
            min="1"
            placeholder={`Payout amount · minimum NGN ${Number(
              partner.minimum_payout || 0
            ).toLocaleString()}`}
            className="min-w-0 flex-1 rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm"
          />
          ...
          <button
            ...
            disabled={
              requestingPayout ||
              Number(partner.available_commission || 0) <= 0
            }
        ...
        {payoutRequests.length > 0 && (
          <div className="mt-5 space-y-2">
            ...
            {payoutRequests.map((request) => (
              <div
                key={request.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-950 p-3 text-sm"
              >
                <div>
                ...
                <span className="rounded-full bg-blue-500/10 px-3 py-1 text-xs font-semibold uppercase text-blue-300">
